...
//Bascially, it goes through all palindrome numbers with 2n digits in a descending order. For each palindrome number u, it checks if the number can be divided by a number x with n digits. If so, u % x should also be a number which has n digits. As it's in descending order, the first number that meets this check is the largest palindrome number we want.
